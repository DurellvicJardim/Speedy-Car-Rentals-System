...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'find') {
  ...
  if ($input_email === '' || !filter_var($input_email, FILTER_VALIDATE_EMAIL)) {
    ...
    $escaped = mysqli_real_escape_string($database_connection, $input_email);
    $sql = "SELECT id, email FROM users WHERE email = '" . $escaped . "' LIMIT 1";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'reset') {
  ...
  if (count($error_messages) === 0) {
    $escaped = mysqli_real_escape_string($database_connection, $found_email);
    $check = mysqli_query($database_connection, "SELECT id FROM users WHERE email = '" . $escaped . "' LIMIT 1");
    ...
    if (!$user) {
      ...
      $hash_esc = mysqli_real_escape_string($database_connection, $hash);
      $upd = "UPDATE users SET password_hash = '" . $hash_esc . "' WHERE id = " . (int) $user['id'];
      $ok = mysqli_query($database_connection, $upd);
      if ($ok) {
        header('Location: login.php?message=Password has been reset. Please log in.');
        exit;
      } else {
        $error_messages[] = 'Could not update password. Please try again.';
        $step = 'reset';
      }
    }
  }
