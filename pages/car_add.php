...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (count($error_messages) === 0) {
    ...
    $sql_insert = "INSERT INTO cars (car_make, car_model, car_year, car_plate, availability, rental_price, image_path)
                       VALUES (
                        '" . mysqli_real_escape_string($database_connection, $car_make) . "',
                        '" . mysqli_real_escape_string($database_connection, $car_model) . "',
                        " . (int) $car_year . ",
                        '" . mysqli_real_escape_string($database_connection, $car_plate) . "',
                        " . (int) $availability_value . ",
                        " . (float) $rental_price . ",
                        '" . mysqli_real_escape_string($database_connection, $image_path) . "'
                       )";
...
<form method="post" class="row g-3" enctype="multipart/form-data">
  ...
  <div class="col-md-8">
    <label class="form-label">Car image (JPG/PNG/GIF)</label>
    <input class="form-control" type="file" name="car_image" accept=".jpg,.jpeg,.png,.gif">
    <div class="form-text">Optional. If provided, it is saved to /images and the path is stored.</div>
  </div>
  <div class="col-12">
